...
ORIG_SRC=/data/ru-en/train.ru
ORIG_TGT=/data/ru-en/train.en
TEST_INPUT=/data/ru-en/test.ru
TEST_REF=/data/ru-en/test.en

function Active_NMT () {
	local ACTIVE_FUNC_1=$1
	local ACTIVE_FUNC_2=$2
	local START_ROUND=$3
	local warm_up=$4
	...
	do	
		if [ "$i" -lt "$warm_up" ]; then
			warm_up_ACTIVE_FUNC_1=random
			warm_up_ACTIVE_FUNC_2=random
		else
			warm_up_ACTIVE_FUNC_1=$ACTIVE_FUNC_1
			warm_up_ACTIVE_FUNC_2=$ACTIVE_FUNC_2
		fi
		# Do active learning
		cd active_data
		...
		do
			previous_num_sents=`cat active_data/unlabeled_${i}_0 | wc -l`
			previous_num_sents=$((previous_num_sents * j))
			echo "CUDA_VISIBLE_DEVICES=$j python3 active.py score -a $warm_up_ACTIVE_FUNC_1 \
				-i ${U}${i}_$j -ref ${ORACLE}${i}_$j \
				-ckpt checkpoints/$i/checkpoint_best_ppl.pth \
				--max_batch_size 0 \
				--tokens_per_batch 16384 \
				--previous_num_sents $previous_num_sents > test_active.out_${ACTIVE_FUNC_1}_${i}_${j}" >> parallel_active.sh
		done
		parallel -j $NGPUS < parallel_active.sh
		rm parallel_active.sh
		mv test_active.out_${ACTIVE_FUNC_1}_${i}_? active_data/
		cat active_data/test_active.out_${ACTIVE_FUNC_1}_${i}_? >> active_data/test_active.out_${ACTIVE_FUNC_1}_$i

		python3 active.py score -a $warm_up_ACTIVE_FUNC_2 \
			-i ${U}${i} -lb ${L}${i} -ref ${ORACLE}${i} \
			-ckpt checkpoint/$i/checkpoint_best_ppl.pth \
			--max_batch_size 0 \
			--tokens_per_batch 16384 > test_active.out_${ACTIVE_FUNC_2}_${i}
		mv test_active.out_${ACTIVE_FUNC_2}_${i} active_data/

		python3 active.py modify -U $U$i -L $L$i.$SRC,$L$i.$TGT --oracle $ORACLE$i -tb $TOK_BUDGET \
			-OU $U$((i+1)) -OL $L$((i+1)).$SRC,$L$((i+1)).$TGT \
			-OO $ORACLE$((i+1)) -AO1 $ACTIVE_OUT${ACTIVE_FUNC_1}_$i -AO2 $ACTIVE_OUT${ACTIVE_FUNC_2}_$i 
		cd active_data
		rm test_active.out*
		rm *_${i}_?
		cd ..
...
function main () {
	...
	ACTIVE_FUNC_1=te
	ACTIVE_FUNC_2=dden
	Active_NMT $ACTIVE_FUNC_1 $ACTIVE_FUNC_2 0 1
	mkdir -p ./result/$ACTIVE_FUNC_2-$ACTIVE_FUNC_1
	mv checkpoints active_data ./result/$ACTIVE_FUNC_2-$ACTIVE_FUNC_1/
	rm -rf data_bin
}
