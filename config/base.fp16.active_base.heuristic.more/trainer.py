...
class Trainer(object):
    ...
    def optimize(self, loss):
        ...
        if config.fp16 == False:
            ...
            if self.n_iter % config.accumulate_gradients == 0:
                ...
                with apex.amp.scale_loss(loss, self.opt.optimizer, delay_unsacle=True) as scaled_loss:
                    scaled_loss.backward()
    ...
    def save_checkpoint(self, name):
        ...
        logger.warning('Saving network parameters ...')
        ckpt["net"] = self.net.state_dict()

        logger.warning("Saving optimizer state_dict...")
        ckpt["opt"] = {}
    ...
    def reload_checkpoint(self, network_only=False):
        ...
        if config.continue_path is not None:
            ...
            logger.warning("Reloading checkpoint from {checkpoint_path} ...")
            ...
            logger.warning("Checkpoint reloaded. Resuming at epoch {self.epoch} / iteration {self.n_total_iter} ...")
