...
function Train_Model () {
	...
	if [ $i -eq 0 ]; then
		...
		local continue_path=checkpoints/$i/checkpoint_best_ppl.pth
		python3 -m torch.distributed.launch --nproc_per_node=$NGPUS train.py \
			--raw_src $L$((i+1)).$SRC \
			--raw_tgt $L$((i+1)).$TGT \
			--continue_path $continue_path \
...
function Test_Model () {
	...
	local SRC=${2:-ru}	
	local TGT=${3:-en}
	TEST_SRC=/data/rufr-en/ru-en/test.$SRC
	TEST_TGT=/data/rufr-en/ru-en/test.$TGT
...
function main () {
	mkdir -p ./result
	for ACTIVE_FUNC in random lc margin te tte; do
