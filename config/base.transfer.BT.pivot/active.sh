...
function Test_Model () {
	...
	python3 translate.py -ckpt checkpoints/$SRC-$TGT/$i/checkpoint_best_ppl.pth \
		...
		--max_batch_size 0 --tokens_per_batch 2000 -k 5 -max_len 200 > checkpoints/$SRC-$TGT/$i/$TEST_DIR/total.out

	cat checkpoints/$SRC-$TGT/$i/$TEST_DIR/total.out | grep ^H | cut -d " " -f3- > checkpoints/$SRC-$TGT/$i/$TEST_DIR/sys.out
	cat checkpoints/$SRC-$TGT/$i/$TEST_DIR/total.out | grep ^T | cut -d " " -f3- > checkpoints/$SRC-$TGT/$i/$TEST_DIR/ref.out

	cat checkpoints/$SRC-$TGT/$i/$TEST_DIR/sys.out | perl -ple 's{(\S)-(\S)}{$1 ##AT##-##AT## $2}g' > checkpoints/$SRC-$TGT/$i/$TEST_DIR/generate.sys
	cat checkpoints/$SRC-$TGT/$i/$TEST_DIR/ref.out | perl -ple 's{(\S)-(\S)}{$1 ##AT##-##AT## $2}g' > checkpoints/$SRC-$TGT/$i/$TEST_DIR/generate.ref

	python3 $FAIRSEQ_PATH/score.py --sys checkpoints/$SRC-$TGT/$i/$TEST_DIR/generate.sys \
		--ref checkpoints/$SRC-$TGT/$i/generate.ref > checkpoints/$SRC-$TGT/$i/$TEST_DIR/bleu.txt
...
function main () {
	...
	BT $ACTIVE_FUNC_1 $ACTIVE_FUNC_2 1000000 11 1 en de 
