...
def main():
    ...
    parser_modify.add_argument('-AO1', '--active_out_1', type=str,
            help="path to active function output 1"
            )
    parser_modify.add_argument('-AO2', '--active_out_2', type=str,
            help="path to active function output 2"
            )
    ...
    if args.mode == "score":
        ...
        if args.input_labeled != None:
            lab_text = []
            f = open(args.input_labeled + '.ru', 'r')
        ...
        # Sort active_out
        assert len(active_out_1) % 4 == 0
        assert len(active_out_1) / 4 == len(oracle)
        active_out_1 = [[active_out_1[i], active_out_1[i+1], float(active_out_1[i+2].split(' ')[-1]), active_out_1[i+3]] for i in range(0, len(active_out_1), 4)]
        active_out_1 = sorted(active_out_1, key=lambda item: item[2])
        ...
        active_out_2 = [[active_out_2[i], active_out_2[i+1], float(active_out_2[i+2].split(' ')[-1]), active_out_2[i+3]] for i in range(0, len(active_out_2), 4)]
        active_out_2 = sorted(active_out_2, key=lambda item: item[2])

        active_out = [0] * len(active_out_1)

        # Add sentence rank under different active functions
        for i in range(len(active_out_1)):
            idx = int(active_out_1[i][3].split()[-1])
            active_out[idx] = [active_out_1[i][0], active_out_1[i][1], i]
        
        for i in range(len(active_out_2)):
            idx = int(active_out_2[i][3].split()[-1])
            assert active_out[idx][0] == active_out_2[i][0]
            assert active_out[idx][1] == active_out_2[i][1]
            active_out[idx][2] += i 
