...
class Trainer(object):
    ...
    def __init__(self, params):
        ...
        train_log = SummaryWriter(os.path.join(config.tensorboard_log_path, "log", "train"))
        valid_log = SummaryWriter(os.path.join(config.tensorboard_log_path, "log", "valid"))
    ...
    def param_grad_to_tensorboard(self, step: int) -> None:
        """
        ...
        for name, param in self.net.named_parameters():
            self._tensorboard.add_train_scalar("parameter_mean/" + name,
                                               param.data.mean(),
                                               step)
            self._tensorboard.add_train_scalar("parameter_std/" + name, param.data.std(), step)
    ...
    def lr_to_tensorboard(self, step: int):
        """
        Send current learning rates to tensorboard
        """
    ...
    def histograms_to_tensorboard(self, step: int) -> None:
        """
        Send histograms of parameters to tensorboard.
        """
        for name, param in self.net.named_parameters():
            self._tensorboard.add_train_histogram(
                    "parameter_histogram/" + name, param, step
                    )


    def metrics_to_tensorboard(self, epoch: int,
                               val_metrics: dict = None) -> None:
        """
        Sends all of the train metrics (and validation metrics, if provided) to tensorboard.
        """
        metric_names = set(valid_metrics.keys())
...
class Enc_Dec_Trainer(Trainer):
    ...
    def train_step(self, raw_batch):
        ...
        if self.n_iter % config.summary_interval == 0 and self.n_iter > 0:
            if config.log_param_and_grad:
                self.param_grad_to_tensorboard(self.n_iter)
            if config.log_lr:
                self.lr_to_tensorboard(self.n_iter)
            self._tensorboard.add_train_scalar("loss/loss_train", loss.item(), self.n_iter)
            if config.log_histograms:
                self.histograms_to_tensorboard(self.n_iter)
    ...
    def valid_step(self):
        ...
        self.metrics_to_tensorboard(self.epoch, scores)
